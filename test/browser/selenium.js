...
const httpServerPs = spawn(`npx http-server ./test/browser -c-1 --no-dotfiles -p 8081`, { shell: true });
...
(async function () {
  let result;
  try {
    result = await runMochaWebDriverTest(webDriverCapabilities, 'http://localhost:8081/index.html?ci=true', {
